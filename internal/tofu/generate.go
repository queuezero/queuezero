...
const (
	defaultFSxCapacityGiB    = 1200       // the FSx-Lustre minimum (and increment)
	defaultFSxDeploymentType = "SCRATCH_2"
)

// storageTmpl is the generator's view of one shared-storage mount, derived from
// a spec.StorageSpec so the template never re-parses Kind. Kind selects which
// resources the template emits; the FSx-only fields are zero for efs entries.
type storageTmpl struct {
	Index     int
	Kind      string // "efs" | "fsx-lustre"
	MountPath string
	Token     string // EFS creation_token / FSx name tag, deterministic per cluster+index

	// fsx-lustre only:
	CapacityGiB    int
	DeploymentType string
	S3ImportPath   string // DRA data-repository path ("s3://..."); empty => no DRA
}

// buildStorage converts the cluster's storage entries into template inputs and
// reports which security groups are needed. efs and fsx-lustre are both emitted;
// any other kind is rejected (spec validation should have caught it first).
func buildStorage(c *spec.Cluster) (out []storageTmpl, hasEFS, hasLustre bool, err error) {
	for i, s := range c.Storage {
		...
		case "fsx-lustre":
			hasLustre = true
			capacity := s.CapacityGiB
			if capacity == 0 {
				capacity = defaultFSxCapacityGiB
			}
			deploy := s.DeploymentType
			if deploy == "" {
				deploy = defaultFSxDeploymentType
			}
			out = append(out, storageTmpl{
				Index:          i,
				Kind:           "fsx-lustre",
				MountPath:      s.MountPath,
				Token:          fmt.Sprintf("%s-fsx-%d", c.Name, i),
				CapacityGiB:    capacity,
				DeploymentType: deploy,
				S3ImportPath:   s.S3Linkage,
			})
		default:
			return nil, false, false, fmt.Errorf("tofu: storage[%d] unknown kind %q", i, s.Kind)
		}
	}
...
func GenerateClusterFoundation(c *spec.Cluster, opts FoundationOpts) (map[string]string, error) {
	...
	storage, hasEFS, hasLustre, err := buildStorage(c)
	if err != nil {
		return nil, err
	}
	data := templateData{
...
// storageTF renders shared filesystems (EFS file systems + per-AZ mount targets,
// and/or FSx-Lustre file systems + optional S3 data-repository associations) plus
// the security group each kind needs. The controller's StateDir lives here
// (durability, §9). The mount-on-boot (fstab/mount) is NOT here — it is the
// deferred S3 bootstrap script's job (§11); this only provisions the filesystem.
...
{{- end}}
{{- if .HasLustre}}
# --- Lustre security group: FSx-Lustre accepts 988 + 1018-1023 from compute+controller ---
resource "aws_security_group" "lustre" {
  name        = "{{.Name}}-lustre"
  description = "queuezero shared FSx-Lustre: ports 988,1018-1023 from compute + controller, egress all"
  vpc_id      = local.vpc_id
  ingress {
    description     = "Lustre 988 from compute"
    from_port       = 988
    to_port         = 988
    protocol        = "tcp"
    security_groups = [aws_security_group.compute.id]
  ...
  ingress {
    description     = "Lustre 988 from controller"
    from_port       = 988
    to_port         = 988
    protocol        = "tcp"
    security_groups = [aws_security_group.controller.id]
  ...
  ingress {
    description     = "Lustre 1018-1023 from compute"
    from_port       = 1018
    to_port         = 1023
    protocol        = "tcp"
    security_groups = [aws_security_group.compute.id]
  ...
  ingress {
    description     = "Lustre 1018-1023 from controller"
    from_port       = 1018
    to_port         = 1023
    protocol        = "tcp"
    security_groups = [aws_security_group.controller.id]
  ...
  tags = { Name = "{{.Name}}-lustre", "q0:cluster" = "{{.Name}}" }
}
{{- end}}
{{- range .Storage}}
{{- if eq .Kind "efs"}}
...
{{- else if eq .Kind "fsx-lustre"}}

# --- Shared FSx-Lustre for mount {{.MountPath}} ({{.DeploymentType}}, {{.CapacityGiB}} GiB) ---
resource "aws_fsx_lustre_file_system" "fsx_{{.Index}}" {
  storage_capacity   = {{.CapacityGiB}}
...
resource "aws_fsx_data_repository_association" "fsx_{{.Index}}" {
  file_system_id       = aws_fsx_lustre_file_system.fsx_{{.Index}}.id
  file_system_path     = "{{.MountPath}}"
  data_repository_path = "{{.S3ImportPath}}"
  s3 {
...
{{- else if eq .Kind "fsx-lustre"}}
output "fsx_{{.Index}}_id" {
  value       = aws_fsx_lustre_file_system.fsx_{{.Index}}.id
  description = "FSx-Lustre file system id for mount {{.MountPath}}."
}

output "fsx_{{.Index}}_dns" {
  value       = aws_fsx_lustre_file_system.fsx_{{.Index}}.dns_name
  description = "FSx-Lustre DNS name for mount {{.MountPath}} (used by the bootstrap script)."
}

output "fsx_{{.Index}}_mountname" {
  value       = aws_fsx_lustre_file_system.fsx_{{.Index}}.mount_name
  description = "FSx-Lustre mount name for mount {{.MountPath}} (the <dns>@tcp:/<mountname> Lustre target)."
}
{{- end}}
{{end}}`
